...
$BaseUrl = 'https://ai-agent-daily.alux.network'
...
foreach ($indexCheck in @(
    @{ html = $chineseIndex; required = @('href="/en/"', 'hreflang="en"', '/assets/alux-mark.png') },
    @{ html = $englishIndex; required = @('href="/"', 'hreflang="zh-CN"', '/assets/alux-mark.png') }
)) {
...
foreach ($chineseReport in $chineseReports) {
    ...
    foreach ($html in @($chineseHtml, $englishHtml)) {
        ...
        foreach ($required in @('site:i18n-head:start', 'site:i18n-nav:start', 'site:issue-footer:start', '/assets/alux-mark.png')) {
