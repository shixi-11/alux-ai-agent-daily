...
async function inspectViewport(browser, name, width, height, maxIntroLines) {
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      introLines: countLines('.intro'),
      dateLines: countLines('.fact:last-child b'),
    };
  });
  ...
  if (result.dateLines !== 1) throw new Error(`${name}: date range wrapped to ${result.dateLines} lines`);
  if (maxIntroLines && result.introLines > maxIntroLines) throw new Error(`${name}: intro wrapped to ${result.introLines} lines`);
...
(async () => {
  ...
  try {
    const wide = await inspectViewport(browser, 'wide', 1920, 1080, 2);
    const desktop = await inspectViewport(browser, 'desktop', 1440, 1000, 2);
    const mobile = await inspectViewport(browser, 'mobile', 390, 844);
    process.stdout.write(`${JSON.stringify({ wide, desktop, mobile }, null, 2)}\n`);
  } finally {
    await browser.close();
